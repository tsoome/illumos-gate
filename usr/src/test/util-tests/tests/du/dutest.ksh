...
function H_flag
{
	...
	egrep "${lineprefix}$(echo $paths1 | sed -e "s/ /$sep/g")$" du.out > egrep.out
	...
	egrep -v "${lineprefix}$(echo $paths1 | sed -e "s/ /$sep/g")$" du.out > egrep.out
	if [[ -s egrep.out ]]
	then
		fail "$fn unexpected output"
	fi
	...
	egrep "${lineprefix}$(echo $paths2 | sed -e "s/ /$sep/g")$" du.out > egrep.out
	if ! cmp -s du.out egrep.out
	then
		fail "$fn unexpected output"
	fi
	...
	egrep -v "${lineprefix}$(echo $paths2 | sed -e "s/ /$sep/g")$" du.out > egrep.out
	if [[ -s egrep.out ]]
	then
		fail "$fn unexpected output"
	fi
...
function s_flag
{
	...
	if [[ $($du -As testdir/testdir1) != $(printf "1\ttestdir/testdir1\n") ]]; then
		fail "$fn unexpected size"
	fi
	if [[ $($du -As testdir/testdir1/B) != $(printf "0\ttestdir/testdir1/B\n") ]]; then
		fail "$fn unexpected size"
	fi
