...
{
	[ -x /usr/bin/digest ] \
	    && /usr/bin/digest -a sha1 "$tarchive" > "$tarchive.hash"
}
...
function create_cpio_archive
{
	...
	if [ $ISA = sparc ] ; then
		...
		# Therefor we have 512B reserved space for disk label,
		...
		cat "$tarchive.head" "$tarchive.cpio" > "$tarchive" \
		    || fatal_error "Cannot upate boot archive"
		...
		# Digest must be created before gzipping the archive.
