 ...
 */
uint32_t nfs_server_vers_min = NFS_VERS_MIN;
uint32_t nfs_server_vers_max = NFS_VERS_MAX;
...
main(int ac, char *av[])
{
	...
	if (proto != NULL &&
		...
		if (NFS_VERS_GETMAJOR(nfs_server_vers_max) == NFS_V4) {
			if (NFS_VERS_GETMAJOR(nfs_server_vers_min) == NFS_V4) {
				fprintf(stderr,
	 ...
	 */
	    ...
	    (nfs_server_vers_min < NFS_VERS_MIN) ||
	    (nfs_server_vers_max > NFS_VERS_MAX))
		usage();
	/*
	...
	protobp->serv = "NFS";
	protobp->versmin = NFS_VERS_GETMAJOR(nfs_server_vers_min);
	protobp->versmax = NFS_VERS_GETMAJOR(nfs_server_vers_max);
	...
	if (NFS_VERS_GETMAJOR(nfs_server_vers_max) > NFS_ACL_V3)
		protobp->versmax = NFS_ACL_V3;
	else
		protobp->versmax = NFS_VERS_GETMAJOR(nfs_server_vers_max);
...
nfssvc(int fd, struct netbuf addrmask, struct netconfig *nconf)
{
	...
	if (strncasecmp(nconf->nc_proto, NC_UDP, strlen(NC_UDP)) == 0) {
		nsa.versmax = (nfs_server_vers_max > NFS_VERS_3) ?
		    NFS_VERS_3 : nfs_server_vers_max;
