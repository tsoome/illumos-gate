...
#undef NDEBUG			/* XXXARM: We want the assertions */
...
undo_reloc(void *vrel, uchar_t *oaddr, uchar_t *iaddr, Reloc *reloc)
{
	...
	assert(0 && "librtld undo_reloc unimplemented");
    ...
    uchar_t *iaddr)
{
	...
	assert(0 && "librtld inc_reloc not implemented");

	if (ELF_R_TYPE(nrel->r_info, M_MACH) == R_AARCH64_JUMP_SLOT) {
		/* LINTED */
		ulong_t	*_oaddr = (ulong_t *)oaddr;
		/* LINTED */
		ulong_t	*_iaddr = (ulong_t *)iaddr;
...
clear_reloc(void *vrel)
{
	...
	rel->r_offset = 0;
	rel->r_info = ELF_R_INFO(0, R_AARCH64_NONE);
	rel->r_addend = 0;
}
