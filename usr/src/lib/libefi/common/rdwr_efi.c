...
efi_reserved_sectors(dk_gpt_t *efi)
{
	...
	return (DEV_BLOCKS(EFI_MIN_RESV_SIZE, efi->efi_lbasize));
}
...
efi_use_whole_disk(int fd)
{
	...
	/*
	 * Initialize resv_index to efi_nparts, so we can use it for search of
	 * data partition.
	 */
	resv_index = efi_label->efi_nparts;

	/* Find reserved partition. */
	for (i = 0; i < efi_label->efi_nparts; i ++) {
		if (efi_label->efi_parts[i].p_tag == V_RESERVED) {
	 ...
	 */
	limit = P2ALIGN(efi_label->efi_last_lba - nblocks - reserved,
	    DEV_BLOCKS(PARTITION_END_ALIGNMENT, efi_label->efi_lbasize));
	 ...
	 */
	if ((resv_index == efi_label->efi_nparts) ||
	    (efi_label->efi_parts[resv_index].p_size != reserved) ||
