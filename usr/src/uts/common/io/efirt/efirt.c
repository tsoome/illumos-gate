...
efirt_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	softp->efi_kfpu = kernel_fpu_alloc(KM_SLEEP);
...
efirt_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	kernel_fpu_free(softp->efi_kfpu);

	mutex_destroy(&softp->efi_mutex);
...
#if 0
static int
efi_enter(struct efirt_state *statep)
{
	mutex_enter(&statep->efi_mutex);
	kernel_fpu_begin(statep->efi_kfpu, 0);
	return (0);
}

static int
efi_exit(struct efirt_state *statep)
{
	kernel_fpu_end(statep->efi_kfpu, 0);
	mutex_exit(&statep->efi_mutex);
	return (0);
}
#endif
