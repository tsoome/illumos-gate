 ...
 */
static int64_t
arc_evict_hdr(arc_buf_hdr_t *hdr, uint64_t *real_evicted) /* XXX */
{
	...
	*real_evicted = 0;
	...
	if (hdr->b_l1hdr.b_bufcnt == 0) {
		...
		if (HDR_HAS_RABD(hdr))
			arc_hdr_free_pabd(hdr, B_TRUE); /* XXX */
...
arc_access(arc_buf_hdr_t *hdr, arc_flags_t arc_flags, boolean_t hit)
{
	...
	if (hdr->b_l1hdr.b_state == arc_anon) {
		 ...
		 * hit, promote the buffer to the MFU state. XXX
		 */
...
arc_read_done(zio_t *zio)
{
	...
	for (acb = callback_list; acb != NULL; acb = acb->acb_next) {
		...
		if (!acb->acb_done || acb->acb_nobuf)	/* XXX */
			continue;
