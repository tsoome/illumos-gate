 ...
 * creating an offset ABD marks the original ABD as the offset's parent.
 *
...
{
	void *c = memalign(64, zfs_abd_chunk_size);
	ASSERT3P(c, !=, NULL);
	return (c);
}
...
abd_free_scatter(abd_t *abd)
{
	size_t n = abd_scatter_chunkcnt(abd);
	for (int i = 0; i < n; i++) {
		free(abd->abd_u.abd_scatter.abd_chunks[i]);
	}
...
abd_alloc_linear(size_t size, boolean_t is_metadata)
{
	...
	abd->abd_u.abd_linear.abd_buf = malloc(size);
...
{
	free(abd->abd_u.abd_linear.abd_buf);
	abd_free_struct(abd);
}
