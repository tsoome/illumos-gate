...
/*
 * dboot_loadfile() will attempt to load the kernel. We only do load one kernel.
 * In case of 64-bit loader, we will allow full 64-bit address space for
 * loading. On error, loader will fall back to multiboot2 boot protocol,
 * and the load_limit is reset to 32-bit address space.
 */
static int
...
dboot_add_framebuffer(struct xboot_info *bi)
{
	...
	gfx = loader_alloc_align(size, 16);
	if (gfx == NULL)
		return (ENOMEM);

	memcpy(gfx, &gfx_fb, sizeof (*gfx));
	gfx->framebuffer_common.mb_type = MULTIBOOT_TAG_TYPE_FRAMEBUFFER;
	gfx->framebuffer_common.mb_size = size;
	...
	if (gfx_fb.framebuffer_common.framebuffer_type ==
	    MULTIBOOT_FRAMEBUFFER_TYPE_INDEXED) {
		uint16_t nc;
		nc = gfx->u.fb1.framebuffer_palette_num_colors;
		memcpy(gfx->u.fb1.framebuffer_palette, cmap,
...
/*
 * We still need to provide multiboot information,
 * specifically the bootp-response and UEFI memory map.
 */
static size_t
mbi_size(void)
{
	...
	size = sizeof (multiboot2_info_header_t);
#if defined(EFI)
#define	EFI_EXTRA_PAGES	3

	size += sizeof (multiboot_tag_efi_mmap_t);
	size += efi_memmap_size();
	...
	/* Include size for end tag. */
	return (size + sizeof (multiboot_tag_t));
}
...
dboot_exec(struct preloaded_file *fp)
{
	...
	bi->bi_uefi_systab = vtop((caddr_t)ST);
#endif
	if (rsdp != NULL)
	...
	mbi_sz = mbi_size();
	mbi = loader_alloc_align(mbi_sz, PAGE_SIZE);
	if (mbi == NULL) {
		rv = ENOMEM;
		goto error;
	}

	/* Set initial size of mbi_tags. */
	mbi->mbi_total_size = 0;
	mbi->mbi_reserved = 0;

	if (bootp_response != NULL) {
		multiboot_tag_network_t *tag;
		tag = (multiboot_tag_network_t *)&mbi->mbi_tags;
		...
		mbi->mbi_total_size =
		    roundup2(mbi->mbi_total_size + tag->mb_size,
		    MULTIBOOT_TAG_ALIGN);
	}
	...
	bi->bi_mb_version = 2;
	bi->bi_mb_info = vtop((caddr_t)mbi);
...
#ifdef EFI
	multiboot_tag_efi_mmap_t *efi_mmap;

	efi_mmap = (multiboot_tag_efi_mmap_t *)
	    ((uintptr_t)&mbi->mbi_tags + mbi->mbi_total_size);

	/* Adjust size to reflect space left. */
	mbi_sz -= sizeof (*mbi);
	mbi_sz -= mbi->mbi_total_size;
	/* Count size of multiboot_tag_efi_mmap_t */
	mbi_sz -= sizeof (*efi_mmap);
	/* Also count end tag */
	mbi_sz -= sizeof (multiboot_tag_t);
	size_t mbi_base_size = mbi->mbi_total_size;
	...
	for (int i = 0; i < 2; i++) {
		...
		mbi->mbi_total_size = mbi_base_size;
		status = efi_get_memory_map(&map_size, &map, &map_key,
		    &desc_size, &desc_ver);
		if (status == EFI_SUCCESS) {
			if (roundup2(map_size, MULTIBOOT_TAG_ALIGN) <= mbi_sz) {
				efi_mmap->mb_type = MULTIBOOT_TAG_TYPE_EFI_MMAP;
				efi_mmap->mb_size = sizeof (*efi_mmap) +
				    map_size;
				efi_mmap->mb_descr_size = desc_size;
				efi_mmap->mb_descr_vers = desc_ver;
				memcpy(efi_mmap->mb_efi_mmap, map, map_size);
				/* Update consumed size. */
				mbi->mbi_total_size =
				    roundup2(
				    mbi_base_size + efi_mmap->mb_size,
				    MULTIBOOT_TAG_ALIGN);
			} else {
				/* It is not fatal error right now. */
				printf("No space in MBI for EFI memory map.\n");
			}
	...
	if (!has_boot_services) {
		...
		mbi->mbi_total_size += tag->mb_size + sizeof (*mbi);
		dev_cleanup();
